...
import nistoar.pdr.preserve.bagit.tools.enhance as tools
...
class TestEnrichReferences(test.TestCase):
    ...
    def test_merge_enhanced_ref(self):
        ...
        self.assertEqual(list(enh.refs.keys())[1], doiu)
    ...
    def test_enhance_existing(self):
        enh = tools.ReferenceEnhancer(rescfg, self.log).enhancer_for(self.bag)
        self.assertEqual(len(enh.refs), 1)
        self.assertNotIn('title', enh.refs[list(enh.refs.keys())[0]])
        self.assertNotIn('citation', enh.refs[list(enh.refs.keys())[0]])

        enh.enhance_existing()
        self.assertEqual(len(enh.refs), 1)
        self.assertIn('title', enh.refs[list(enh.refs.keys())[0]])
        self.assertIn('citation', enh.refs[list(enh.refs.keys())[0]])
    ...
    def test_remove_missing_from(self):
        ...
        self.assertEqual(list(enh.refs.keys())[0], doiu)
        self.assertEqual(list(enh.refs.keys())[1], nondoi)
        self.assertEqual(list(enh.refs.keys())[2], "_#noloc")
