...
from nistoar.pdr.utils import webrecord as webrec
...
class TestWebRecorder(test.TestCase):
    ...
    def test_parser_byrec_records(self):
        ...
        with open(self.recfile) as fd:
            ...
            rec = next(iter)
            self.assertIn("/foo/bar", next(rec))
        
            rec = next(iter)
            self.assertIn("/foo/gurn", next(rec))

            rec = next(iter)
            self.assertIn("/foo/goob", next(rec))
    ...
    def test_parser_parse_record(self):
        ...
        with open(parser._recfile) as fd:
            ...
            recs = byrec.records()
            rec = parser._parse_record(next(recs))

            self.assertEqual(rec.op, "POST")
            self.assertEqual(rec.resource, "/foo/bar")
            ...
            self.assertEqual(rec.resource, "/foo/gurn")
            self.assertTrue(rec.time)
            self.assertEqual(rec.service, "WebRec")
            self.assertEqual(rec.headers, [])
            self.assertEqual(rec.body, "")
