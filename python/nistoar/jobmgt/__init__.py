"""
a module for managing jobs that launch as separate, potentially long-running processes.  

This module provides the following features and capabilities:
  ...
  * A callback function can be provided to run after each job completes, successfully or not.
...
A :py:class:`JobQueue` constructor can accept a callback function that will get called after each 
Job exits (successfully or not) and which runs in the same Python virtual machine as that of the 
queue's client.  One can also provde a post-execution function in the execution module if it is 
important for a callback to run in the Job process itself.  
...
class JobQueue:
    ...
    The work that a queue is supposed to apply to a data job is provided at construction time via
    an _execution module_.  This module must contain a ``process()`` function.  This function must 
    accept the following arguments:

    _dataid_
        (str) a data-unique name for the data that will be operated on.  The implementation must be 
        able to map this name to all the necessary input data (via disk, typically).
    _config_
        (dict) the configuration data generated by merging the configuration provided to 
        :py:meth:`submit` and the configuration provided when the queue was created.
    _args_
        (list[str]) a list of arguments that was passed to the job process.  These are the arguments
        that were provided to :py:meth:`submit`.
    _log_
        (Logger) a Logger that the job process should use to record messages

    This module _may_ also include a ``notify()`` function.  If provided, it will be called after 
    the ``process()`` fucntion completes (successfully or not).  This function must accept the 
    following arguments:

    _jobfile_
        (str) the path to the Job's persistent state file.  The function can open this file to 
        get the key information and outputs from the job.
    _config_
        (dict) the same configuration that was passed to the ``process()`` function
    _log_
        (Logger) a Logger that the job process should use to record messages
    ...
    def __init__(self, queuename: str, queuedir: Union[Path,str], execmodule: Union[ModuleType,str],
                 config: Mapping=None, log: Logger=None, callback: Callable=None, resume: bool=True):
        """
        ...
        :param Callable callback:   a function to call after each job completes.  This function will
                                    be passed two arguments: the Job object for the completed job,
                                    and its integer exit code.  An exit code of -1 indicates that 
                                    the Job failed to start or was prematurely killed before it could 
                                    exit (the log will indicate which).  
        ...
        if not log:
            log = logging.getLogger("JobQueue:"+self.name)
        self.log = log
        if callback and not isinstance(callback, Callable):
            raise TypeError("JobQueue: callback is not a Callable: "+str(type(callback)))
        self.post_job_callback = callback

        self.pq = queue.PriorityQueue()
        self.runner = JobRunner(self.name, self.qdir, self.pq, self.log.getChild("runner"), 
                                self.cfg.get("runner"), self.post_job_callback)
...
class JobRunner:
    ...
    def __init__(self, qname: str, jobdir: Path, jobq: queue.Queue, log: Logger=None,
                 config: Mapping=None, callback: Callable=None):
        self.qname = qname
        self.jdir = jobdir
        self.jq =jobq
        self.runthread = None
        ...
        self.cfg = config
        if callback and not isinstance(callback, Callable):
            raise TypeError("JobRunner: callback is not a Callable: "+str(type(callback)))
            ...
            async def __call__(self):
                while not self.jq.empty():
                    ...
                    try:
                        ...
                        if self.runner._callback:
                            try:
                                self.runner._callback(job, ec)
                            except Exception as ex:
                                self.runner.log.error("job callback exception: "+str(ex))
