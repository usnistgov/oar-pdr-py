...
[ -n "$OAR_WORKING_DIR" ] || OAR_WORKING_DIR=`mktemp -d _midasserver.XXXXX`
[ -d "$OAR_WORKING_DIR" ] || {
    echo midasserver: ${OAR_WORKING_DIR}: working directory does not exist
    exit 10
}
[ -n "$OAR_LOG_DIR" ] || export OAR_LOG_DIR=$OAR_WORKING_DIR
[ -n "$OAR_MIDASSERVER_CONFIG" ] || OAR_MIDASSERVER_CONFIG=/apps/midas-config.yml
...
opts=
oar_midas_db_type=$1
[ -z "$oar_midas_db_type" ] || opts="--set-ph oar_midas_db_type=$oar-midas_db_type"
