"""
the uWSGI script for launching the MIDAS-specific service layer of the 
filemanager.

This script launches the web service using uwsgi.  For example, one can 
launch the service with the following command:

  uwsgi --plugin python --http-socket :9090 --wsgi-file fm-uwsgi.py \
        --set-ph oar_config_file=fm_conf.yml

Parameters provided via ``--set-ph`` are optional.  If ``oar_config_file``
is not set, this script will attempt to pull ocnfiguration data from a 
configuration server.

This script pays attention to the following environment variables:
   ...
   OAR_CONFIG_APP      The name of the component/application to retrieve 
                          configuration data for (default: midas-fm);
                          this is only used if OAR_CONFIG_SERVICE is used.

See the nistoar.midas.dap.fm.flask module for a description of the configuration 
...
from nistoar.midas.dap.fm import flask
...
if confsrc:
    ...
    srvc.wait_until_up(int(_dec(uwsgi.opt.get('oar_config_timeout', 10))),
                       True, sys.stderr)
    cfg = srvc.get(_dec(uwsgi.opt.get('oar_config_appname', 'midas-fm')))
...
application = flask.create_app(cfg)
logging.info("file manager's MIDAS layer ready")
